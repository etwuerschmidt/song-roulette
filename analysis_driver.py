...
feature_plot = Plotter().radar_graph(avg_audio_features, display=True)
...
day_plot = Plotter().line_graph(day_count.keys(), day_count.values(), display=True)
user_count = AnalysisClient.AnalysisClient.track_count_per_user(all_tracks)
user_plot = Plotter().bar_graph([f"User {key}" for key in user_count.keys()], user_count.values(), display=True)
